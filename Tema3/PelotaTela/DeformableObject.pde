public class DeformableObject
{
  ...
  void createDeformableObject(){
    for(int i = 0; i < N_H; i++)
    {
      for(int j = 0; j < N_V; j++)
      {  
          if((i == 0 && j == 0 || i == 0 && j == N_V-1) && areClamped) 
            _nodes[i][j] = new Particle(new PVector(i*_sepH, j*_sepV, 0), new PVector(0,0,0), PARTICLE_MASS, true);
          else
            _nodes[i][j] = new Particle(new PVector(i*_sepH, j*_sepV, 0), new PVector(0,0,0), PARTICLE_MASS, false);
      }
    }
  }
  ...
  void createBend()
  {
    for (int y = 0; y < _numNodesV; y++){
      for (int x = 0; x < _numNodesH; x++) {
        ...
        if((x == 0 && y == 0 || x == 0 && y == N_V-1) && areClamped)
